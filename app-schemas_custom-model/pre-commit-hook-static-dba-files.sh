...
# Usage:
#
#   Place this script in the same directory as config.xsson, and update
#   config.xsson to have the following value:
#
...
# The schema-tool has to be run from the directory where config.xsson lives,
...
# config.xsson (which is the same real path as this hook), but before doing so
...
STATIC_ALTER_DIR=$(/usr/bin/env python2.7 -c "import json; print json.loads(open('config.xsson').read())['static_alter_dir']")
if [[ $? -ne 0 ]]
then
  echoerr 'No static_alter_dir property found in config.xsson, but is required.'
  exit 1
fi
